<div class="modal fade" id="modalAprobado{{ $solicitud->id }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="login-box-body" >
      <div class="modal-header" >
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>  
        <h4 class="modal-title text-center"   style="color:yellow" id="modalAprobado"><b>INSERTAR NOTA DE APROBACIÓN</b></h4>
...
@unless(request()->is('/solicitudes'))
  <script>
    ...
    {
       $('#modalAprobado').modal({ 'show' : {{ count($errors) > 0 ? true : false }}  });
    }

    $('#modalAprobado').on('hide.bs.modal', function(){
      //console.log('El modal se cierra');
      window.location.hash = '#';
    });

    $('#modalAprobado').on('show.bs.modal', function(){
